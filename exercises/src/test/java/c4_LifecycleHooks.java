...
public class c4_LifecycleHooks extends LifecycleHooksBase {
  ...
  public void no_subscription_no_gains() {
    ...
    Flux<Integer> temperatureFlux = room_temperature_service().doOnSubscribe(x -> hooksTriggered.add("subscribe"));
  ...
  public void atomic_counter() {
    ...
    //todo: change this line only
    Flux<Integer> temperatureFlux = room_temperature_service().doOnNext(x -> {
  ...
  public void one_to_rule_them_all() {
    ...
    //todo: change this line only
    Flux<Integer> flux = Flux.just(1, 2, 3).doOnEach(x -> signals.add(x.getType().name()));
