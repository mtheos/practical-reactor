...
public class c13_Context extends ContextBase {
  ...
  public void message_tracker() {
    ...
    StepVerifier.create(mono)
        .expectNextMatches(m -> m.correlationId.equals("2-j3r9afaf92j-afkaf") && m.payload.equals("Hello World!"))
        .verifyComplete();
  }
  ...
  public void pagination() {
    //todo: set this field when error occurs
    ...
    Flux<Integer> results = Mono.deferContextual(ctx -> getPage(ctx.get(AtomicInteger.class).get()))
        .doOnEach(signal -> {
          var counter = signal.getContextView().get(AtomicInteger.class);
          switch (signal.getType()) {
            case ON_NEXT:
              counter.incrementAndGet();
              break;
            case ON_ERROR:
              pageWithError.set(counter.getAndIncrement());
              break;
            default:
              break;
          }
