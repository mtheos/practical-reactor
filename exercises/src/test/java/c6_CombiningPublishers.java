...
public class c6_CombiningPublishers extends CombiningPublishersBase {
  ...
  public void mail_box_switcher() {
    //todo: feel free to change code as you need
    Flux<Message> myMail = mailBoxPrimary().switchOnFirst((first, primary) -> first.get().metaData.equals("spam") //
                                                                              ? mailBoxSecondary() //
                                                                              : primary);
  ...
  public void acid_durability() {
    //todo: feel free to change code as you need
    Flux<String> committedTasksIds = tasksToExecute().concatMap(task -> task.flatMap(id -> commitTask(id).thenReturn(id)));
  ...
  public void car_factory() {
    ...
    StepVerifier.create(producedCars)
        ...
        .expectRecordedMatches(cars -> cars.stream()
            .allMatch(car -> Objects.equals(car.chassis.getSeqNum(), car.engine.getSeqNum())))
        .verifyComplete();
  }
  ...
  public void cleanup() {
    ...
    //todo: feel free to change code as you need
    Flux<String> stream = Flux.usingWhen(StreamingConnection.startStreaming(), x -> x, t -> StreamingConnection.closeConnection());
