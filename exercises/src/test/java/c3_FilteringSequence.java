...
public class c3_FilteringSequence extends FilteringSequenceBase {
  ...
  public void girls_are_made_of_sugar_and_spice() {
    ...
    Flux<String> shortListed = popular_girl_names_service().filter(x -> x.length() <= 4);

    StepVerifier.create(shortListed).expectNext("Emma", "Ava", "Mia", "Luna", "Ella").verifyComplete();
  }
  ...
  public void not_a_binary_search() {
    ...
    StepVerifier.create(numbers).expectNextMatches(i -> i >= 200).expectNextCount(99).verifyComplete();
  }
  ...
  public void golden_middle() {
    ...
    StepVerifier.create(numbers).expectNextMatches(i -> i >= 100).expectNextCount(99).verifyComplete();
  }
}
